...
async def fetch_trades(start_time: int, exchange_ids: List[str]):
    ...
    for i, trades in enumerate(trades_list):
        ...
        for (company_exchange_id, symbol), group in grouped_df:
            ...
            trade_summary['no_trades'] = len(df)
            trade_summary['vol_base'] = df['base_notional'].sum()
            trade_summary['vol_quote'] = df['quote_notional'].sum()
            # Ensure 'entry_price' and 'base_notional' are available in your DataFrame
            trade_summary['avg_price'] = (df['entry_price'] * df['base_notional']).sum() / df['base_notional'].sum()
            trade_summary['fees_usd'] = df['commission_usd'].sum()
            trade_summary['maker'] = df['order_type'].value_counts(normalize=True).get('maker', 0)
            print(trade_summary)
